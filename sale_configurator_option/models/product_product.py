...
class ProductProduct(models.Model):
    ...
    used_on_option_ids = fields.One2many("product.configurator.option", "product_id")
    ...
    def _compute_used_on_product_ids(self):
        for record in self:
            record.used_on_product_ids = record.mapped(
                "used_on_option_ids.used_on_product_tmpl_ids.product_variant_ids"
            )
    ...
    def action_archive(self):
        for record in self:
            options = record.used_on_option_ids
            if options:
                self.message_post(
                    body=_(
