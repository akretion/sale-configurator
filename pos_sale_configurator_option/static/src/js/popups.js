odoo.define('pos_sale_configurator_option.views', function (require) {
    ...
    var SelectConfigOptionPopupWidget = PopUpWidget.extend({
        ...
        change_qty: function(evt) {
            ...
            var delta = parseInt(
                clicked_input.dataset['delta'] || clicked_input.parentElement.dataset['delta']
                , 10); //clicked_input can be "+" or "button"
            delta = delta ? delta : 0;
            output_qty.value = Math.max(parseInt(output_qty.dataset['min'], 10), 
                Math.min(
                    parseInt(output_qty.value, 10) + delta,
                    parseInt(output_qty.dataset['max'], 10)) //limit input to max_qty
            ); //no negative qty
    ...
    gui.define_popup({name:'select-config-option', widget: SelectConfigOptionPopupWidget});
    ...
    ppt.VariantListWidget.include({
        ...
        init: function(parent, options) {
            ...
            this.click_variant_handler = function(event) {
                ...
                var last_orderline =
                    order.pos.get('selectedOrder').get_last_orderline();
